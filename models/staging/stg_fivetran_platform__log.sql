...
final as (
    ...
    select
        ...
        case when transformation_id is not null and event is null then 'TRANSFORMATION'
            when event like '%sdk%' then 'CONNECTOR_SDK'
        ...
        when event like '%sdk%' and message_data like '%has succeeded%' then 'connector sdk run success'
        when event like '%sdk%' and message_data like '%has failed%' then 'connector sdk run failed'
